...
use super::math::{
    closest_point_on_line_to_ray, perpendicular_basis, ray_plane_intersect, screen_ray,
};
...
impl TransformGizmo {
    pub fn begin_drag(&mut self, axis: Axis, mouse: Vec2, camera: &Camera, viewport: (f32, f32)) {
        ...
        match self.mode {
            GizmoMode::Translate => {
                let hit = self
                    .translate_drag_point(axis, mouse, camera, viewport)
                    .unwrap_or(self.position);
                self.drag = Some(DragState::Translate {
                    anchor_offset: hit - self.position,
                });
            }
            GizmoMode::Rotate => {
    ...
    pub fn drag(&mut self, mouse: Vec2, camera: &Camera, viewport: (f32, f32)) {
        ...
        match *state {
            ...
            DragState::Scale {
                ...
                let delta_px = mouse.y - start_mouse.y;
    ...
    fn angle_on_plane(
        ...
        let hit =
            ray_plane_intersect(origin, dir, self.position, u.cross(v)).unwrap_or(self.position);
        let rel = hit - self.position;
        rel.dot(v).atan2(rel.dot(u))
    }

    fn translate_drag_point(
        &self,
        axis: Axis,
        mouse: Vec2,
        camera: &Camera,
